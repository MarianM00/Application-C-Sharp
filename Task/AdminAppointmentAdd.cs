...
namespace Task
{   
    public partial class AdminAppointmentAdd : Form
    {
        ...
        private void bunifuButton1_Click(object sender, EventArgs e)
        {
            try
            {
...

                {
                    ...
                    String query = "INSERT INTO dbo.Appointments (CodeDoctor,CodePatient,App_Date) VALUES (@CodeDoctor,@CodePatient,@App_Date)";

                    using (SqlCommand command = new SqlCommand(query, con))
                    {
                        command.Parameters.AddWithValue("@CodeDoctor", TxtCodeDoctor.Text);
                        
                        command.Parameters.AddWithValue("@CodePatient", TxtCodePatient.Text);
                        command.Parameters.AddWithValue("@App_date", TxtApp_Date.Text);
                        ...
                        if (result < 0)
                        {
                            MessageBox.Show("error", "Save with successfully", MessageBoxButtons.OK, MessageBoxIcon.Error);
